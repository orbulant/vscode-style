...
const loadJsFile = async (path, callback, isImport) => {
    const resp = await fetch((path.startsWith('./') ? path : basePath + path) + ".js")
...
const replaceCommandPannelHeight = (text) => {
    ...
    text = text.replace(/(44:)22(?=})/g, (_, prefix) => `${prefix}${commandPannelHeight}`)
    return text
}
...
(async function () {
    ...
    await loadJsFile('./workbench', async (text) => {
